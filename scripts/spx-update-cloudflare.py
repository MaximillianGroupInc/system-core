...
def fetch_ip_ranges(url: str) -> list[str]:
    ...
    for line in lines:
        ...
        if not candidate or candidate.startswith("#"):
            continue
        try:
            cidr_ranges.append(str(ipaddress.ip_network(candidate, strict=False)))
        except ValueError:
            print(f"ERROR: Invalid CIDR from {url}: {candidate}", file=sys.stderr)
            sys.exit(1)
...
def build_config(v4_ranges: list[str], v6_ranges: list[str]) -> str:
    ...
    lines = [
        ...
        f"# Auto-generated by scripts/spx-update-cloudflare.py on {timestamp}",
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print(config)
        print("WARNING: Also update nginx/nginx.conf geo $from_cloudflare ranges to keep origin gate in sync.", file=sys.stderr)
        return
